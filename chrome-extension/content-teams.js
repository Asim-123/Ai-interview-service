...
console.log('🎯 AI Interview Copilot - Teams mode activated');
...
function observeChatMessages() {
  const chatContainer = document.querySelector('.ui-chat__messagelist') ||
  ...
  console.log('✅ Teams chat container found, observing messages...');
...
function processChatMessage(element) {
  const messageElement = element.querySelector('[class*="message-body"]') || element;
  ...
  if (detection.isQuestion) {
    console.log('💬 Question detected in Teams chat:', messageText);
    sendQuestionToBackend(messageText, 'chat', 'teams');
  }
}
...
function observeCaptions() {
  const captionContainer = document.querySelector('[class*="caption"]') ||
                          document.querySelector('[data-tid="closed-captions"]');
  ...
  console.log('✅ Teams caption container found, observing captions...');
...
function processCaptionSentence(sentence) {
  ...
  if (detection.isQuestion) {
    console.log('📝 Question detected in Teams captions:', sentence);
    sendQuestionToBackend(sentence, 'caption', 'teams');
  }
}

// MEETING DETECTION
function detectMeetingStatus() {
  const controlBar = document.querySelector('[class*="calling-controls"]') ||
                    document.querySelector('[data-tid="calling-controls"]');
  const inMeeting = !!controlBar;

  if (inMeeting && !meetingNotified) {
    meetingNotified = true;
    console.log('✅ Teams meeting detected — notifying Ghost Interviewer');
    sendMeetingEventToBackend('teams', 'meeting_start');
  }
